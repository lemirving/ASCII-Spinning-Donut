...
thetaSpacing = 10
phiSpacing  = 1
chars = ".,-~:;=!*#$@" # indice que controla a iluminação
...
while run:
    ...
    for j in range(0,628,thetaSpacing):
        for i in range(0,628,phiSpacing):
            ...
            n = math.sin(B)
            t = c * h * g - f * e
            x = int(xOffset + 40 * D * (l * h * m - t * n)) #coordenada do x após rotação
            y = int(yOffset + 20 * D * (l * h * n + t * m))# coordenada do y após rotação

            o = int(x + columns * y)
            N = int(8 * ((f * e - c * d * g) * m - c * d * e - f * g - l * d * n)) # index de iluminação
